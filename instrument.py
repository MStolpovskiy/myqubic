...
from qubic.warnings import QubicDeprecationWarning, warn
...
class QubicInstrument(Instrument):
    ...
    def _init_deprecated_sky(self, name, band, nside):
        names = name.split(',')
        kind = 'I' if 'nopol' in names \
                   else 'QU' if 'qu' in names \
                             else 'IQU'
        sky = QubicScene(band, kind=kind, nside=nside)
...
def _peak_angles(q, scene, i_band=None, kmax=5):
    """
    ...
    ----------
    i_band : int, optional
        For two bands configuration, i_band represents index of focal plane
    kmax : int, optional
...
def _peak_angles_fraction(q, scene, fraction, i_band=None):
    # there is no need to go beyond kmax=5
    theta, phi = _peak_angles(q, scene, i_band=i_band, kmax=5)
